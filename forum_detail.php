<?php
    ...
    if (!isset($_SESSION["LoggedIn"]) && !isset($_SESSION["UserId"])) {
	 ...
	 if(!isset($_GET['id_nf'])){
        die("Error: ID News Forum tidak ditemukan");
    }
	 else {
        $id_news = $_GET['id_nf'];
        $db_f = new Forum();
        $res_f = $db_f->RetriveSingleNewsForus($id_news);
	 }    
...
<div id="templatemo_right_column">
    ...
    <div id="templatemo_content">
      <h1>News Forum</h1>
        ...
        <div class="templatemo_wedo">
          <?php 
            if(sizeof($res_f) > 0) {
          ?>
          <p class="ttx"><b><?php echo $res_f['discusion'];  ?></b></p>
          <p class="ttx">by : <?php echo $res_f['nama']; ?> ( <?php 
          $datee = date_create($res_f['lastpost']); 
          echo date_format($datee, 'D, d-M-Y H:i:s'); 
      ...
      <?php } ?>
        ...
        <p><img src="images/templatemo_line.jpg" alt="templatemo.com" width="100%" height="2" /></p>
        <!--
        <p style="text-align:right"><a href='reply_news_forum.php?idn=<?php //echo $id_news?>' >reply</a></p>
        -->
        <?php 
          if($_SESSION['role']=="2") {
              ?>
                <p style="text-align:right"><a href='delete_news_forum.php?idn=<?php echo $id_news?>'
          onclick="return confirm('Are you sure want to delete this News Forum?')">Delete</a></p>
              <?php    
          }
        ?>
