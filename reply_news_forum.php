<?php
    ...
    if($_POST) {
        ...
        $reply_by = $_SESSION['UserId'];
	    	$db_f = new Forum();
	    	$res_f = $db_f->ReplyNewsForum($id_news, $message, $reply_by);
	   		if($res_f) {
                echo "<script type=\"text/javascript\">
                                alert(\"".'Success adding Reply'."\");
                                window.location = \"forum_detail.php?id_nf=".$id_n."\"
                            </script>";
	   		} else {
	   			echo "<script type=\"text/javascript\">
                                alert(\"Failed adding Reply.\");
                                window.location = \"forum_detail.php?id_nf=".$id_n."\"
                            </script>";	
	   		}
	   	}  catch(Exception $e){
    ...
    <div id="templatemo_content">
      <h1>News Forum</h1>
        ...
        <div class="templatemo_wedo">
          <form method="post">
            <p><h1>Reply Message</h1></p>
	          <p><textarea class="text" cols="86" rows ="20" name="message" required></textarea></p>
